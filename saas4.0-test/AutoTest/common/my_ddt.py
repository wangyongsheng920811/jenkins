...
IS_COMMON = 'common_test'  # if not 0, use one common test case to auto run all json datas
...
def file_data(common_test=0):
    ...
    def wrapper(func):
        ...
        value = file_path.replace(os.sep + 'case' + os.sep, os.sep + 'data' + os.sep).replace('.py', '.json')
        # value = re.sub(re.compile(r"test[^\\]*py$", re.S), func.__name__ + ".json", file_path).replace('case', 'data')
        setattr(func, FILE_ATTR, value)
        setattr(func, IS_COMMON, common_test)
        return func
    return wrapper
...
def add_test(cls, test_name, test_docstring, func, *args, **kwargs):
    ...
    """
    setattr(cls, test_name, feed_data(func, test_name, str(func.__doc__) + ': ' + test_docstring, *args, **kwargs))


def process_file_data(cls, name, func, file_attr, tag_attr):
    """
    ...
    with codecs.open(data_file_path, 'r', 'utf-8') as f:
        ...
        if _is_yaml_file:
            ...
            data = json.load(f)
            _add_common_tests_from_data(cls, name, func, data)
        else:
            data = json.load(f).get(func.__name__)
...
def _add_tests_from_data(cls, name, func, data):
    ...
    for i, elem in enumerate(data):
        if isinstance(data, dict):
            ...
            test_name = func.__name__ + '_' + key
            test_desc = value.get('desc')
        elif isinstance(data, list):
            value = elem
            test_name = mk_test_name(name, value, i)
        if 'test_common' not in key:
            if isinstance(value, dict):
                value.update({'path': path})
                add_test(cls, test_name, test_desc, func, **value)
            else:
                add_test(cls, test_name, test_name, func, value)


def _add_common_tests_from_data(cls, name, func, data):
    """
    Add common tests from data loaded from the data file into the class
    """
    # path = data.get('path')
    # del data['path']
    for i, elem in enumerate(data):
        if isinstance(data, dict):
            case_name, case_data = elem, data[elem]
            path = case_data.get('path')
            del case_data['path']
            for i, elem in enumerate(case_data):
                if isinstance(case_data, dict):
                    key, value = elem, case_data[elem]
                    test_name = case_name + '_' + key
                    test_desc = value.get('desc')
                elif isinstance(case_data, list):
                    value = elem
                    test_name = mk_test_name(name, value, i)
                if 'test_common' in key:
                    if isinstance(value, dict):
                        value.update({'path': path})
                        add_test(cls, test_name, test_desc, func, **value)
                    else:
...
def ddt(cls):
    ...
    for name, func in list(cls.__dict__.items()):
        if hasattr(func, DATA_ATTR):
            ...
            tag_attr = getattr(func, IS_COMMON)
            process_file_data(cls, name, func, file_attr, tag_attr)
            delattr(cls, name)
    return cls
